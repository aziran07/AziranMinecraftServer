...
메인은 다운로드·주소 복사와 플레이 가이드 선택에 집중한다. 상세 설명은 다음 페이지로 나눈다.

| 페이지 | 내용 |
| --- | --- |
| `index.html` | 다운로드, 주소 복사, 지도·명령어·Occultism·배낭·정렬·화면 설정 카드 |
| `join.html` | 준비물과 설치 4단계, Java 설정 이미지, 기존 팩 업데이트 |
| `play.html` | 배낭, 인벤토리 정렬, JourneyMap과 관련 가이드 |
| `visuals.html` | 셰이더, 리소스팩, 한국어 번역 설정 |
| `help.html` | 두 번째 실행 멈춤 해결, Java 안내, Discord 문의 |
| `commands.html`, `occultism.html` | 명령어와 Occultism 입문 안내 |

기존 메인 앵커는 짧은 안내 링크에 남기고, 사이트 내부 링크는 새 페이지를 직접 가리킨다.
설계·검증 기준은 [사이트 정보 구조](SITE_INFORMATION_DESIGN.md)에 기록한다.
...
python3 -m unittest discover -s tests -p test_site_links.py -v
```

화면 변경 시 로컬 `site/`를 HTTP로 제공해 360px·768px·1280px에서 주요 행동,
카드·메뉴 이동, 이미지, 가로 넘침과 키보드 포커스를 확인한다.
...
### 2026-09-25 정보 구조 개편 검증

Claude가 `site/`를 구현하고 Codex가 로컬 HTTP 서버와 별도 Chromium으로 확인했다.
7개 페이지 × 360·768·1280px에서 가로 넘침, 이미지 누락, 스크립트·HTTP 오류가 없었다.
1280px 메인 높이는 6,279px에서 1,217px로, 브라우저가 표시한 본문 텍스트는
6,406자에서 813자로 줄었다. 플레이 가이드가 y=525px에서 시작하며 첫 줄에 지도·명령어·Occultism이 보인다.
360px 메인은 2,275px이고 가이드는 y=885px에서 시작한다.

로컬 링크·앵커·자산 검사 1개, 주소 복사 검사 2개, JavaScript 구문 검사가 통과했다.
실제 클립보드에 `mc.aziran.uk`가 복사되는지, 키보드 건너뛰기·포커스 표시,
기존 북마크에서 설치·배낭·리소스팩·문제 해결로 이동하는지와 가이드 카드 왕복 이동을 확인했다.
기존 외부 링크가 빠지지 않았고 이동한 본문의 설치·호환·미검증 설명도 대조했다.
Orca 내장 브라우저는 연결 종료·탭 소실로 사용하지 못했으나 별도 Chromium 검증은 완료했다.
이번 변경은 문서·웹 안내만 대상으로 하며 게임 내 기능 시험은 포함하지 않는다.
