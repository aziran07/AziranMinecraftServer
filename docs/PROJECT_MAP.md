...
`AGENTS.md`의 Orca 절차를 따릅니다. Codex는 대상 기능·근거 파일, 원인·요구사항, 설계와 제외 범위, 수정 소유권, 관찰 가능한 수용 기준, 검증 환경·명령, 데이터 보호 조건을 전달합니다. Claude는 구현과 로컬 검증을 수행하고 결과를 Orca orchestration으로 반환합니다. Codex는 실제 diff와 독립 검증으로 완료를 판단하고 결함은 Claude에 돌려보냅니다.

Orca 명령은 이 문서에 고정하지 않습니다. 작업 시 설치된 `orca-cli`·`orchestration` 스킬과 버전이 맞는 CLI 가이드를 읽습니다. 문서·분석은 Codex가 직접 수행할 수 있으며 운영 설정·스크립트 수정은 Claude 구현 범위입니다.
