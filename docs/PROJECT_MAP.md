...
주소 복사는 `tests/test_join_guide_copy.cjs`, 로컬 링크·앵커·자산은 `tests/test_site_links.py`로 검증한다.
...
- **백업 범위:** 정기 백업과 배포 전 보호 백업 모두 현재 서버의 `world/`(모든 차원)만 포함합니다(2026-09-25 사용자 결정, `AGENTS.md`의 Backup Scope). Modonomicon 2.7.0 배포 때 만든 전체 데이터 백업은 사용자 요청으로 같은 날 영구 삭제했습니다. 모드·서버 설정·BlueMap 렌더 데이터·관리 서비스 데이터까지 복구하는 전체 백업은 아닙니다. 같은 호스트의 `minecraft_backups/`에 두므로 호스트 장애 대비용 외부 백업도 아닙니다. 저장 동기화와 실패 처리, cron 일정, 복원 절차는 [BACKUPS.md](BACKUPS.md)에 기록합니다.
...
`scripts/build_client_pack.py`는 서버 고정 목록과 `mods-26.3-client-extra.lock.json`을 읽어 클라이언트 모드 21개, 셰이더 팩 1개와 리소스팩 3개(팩 `1.1.10`)를 Modrinth `.mrpack`, 수동 설치 ZIP, MultiMC 인스턴스 ZIP 세 가지로 패키징한다. MultiMC ZIP의 로더 컴포넌트 UID는 MultiMC 공식 메타의 `net.neoforged`이며 Prism Launcher의 `net.neoforged.neoforge`와 다르다. 클라이언트 전용 JAR 캐시는 `client-mods-cache/`, 생성 파일은 `dist/`에 두며 둘 다 Git에서 제외한다. 생성된 목록은 `mods-26.3-client.lock.json`, 검증은 `python3 -m unittest discover -s tests -v`를 사용한다. 설치 안내는 [README](../README.md#클라이언트-모드팩), 선택 근거는 [호환성 검토](CLIENT_MOD_COMPATIBILITY_26_3.md)에 있다. 클라이언트 전용 모드(ImmediatelyFast, Mouse Tweaks, JourneyMap, Sodium, Iris, Nemo's Inventory Sorting)와 셰이더 팩은 서버에 설치하지 않으므로 이 빌드는 서버 구성·모드 디렉터리를 바꾸지 않는다. 재빌드는 이전 버전의 MultiMC 인스턴스 ZIP만 `gio trash`로 휴지통에 보내며(복구 가능, `gio` 없으면 빌드 중단), `.mrpack`과 수동 ZIP은 지우지 않는다.

입력 lock이 `bundle_jar=false`로 표시한 모드는 JAR을 산출물에 담지 않는다. `.mrpack`의 Modrinth CDN 다운로드 항목으로만 설치되고, 수동 ZIP·MultiMC ZIP·`manifest.json`에서는 빠진다. `1.1.4`~`1.1.6`에서는 JourneyMap과 입력 lock `shaderpacks`의 Complementary Reimagined가 여기에 해당한다. [JourneyMap 공식 라이선스](https://teamjm.github.io/journeymap-docs/6.0.x/about/licensing/)가 번들·재호스팅을 금지하고 설치·실행 중 CurseForge·Modrinth 다운로드만 허용하며, Complementary License Agreement 1.7의 1.2.d도 Modrinth·CurseForge 시스템으로만 모드팩에 넣도록 한다. 그래서 두 ZIP은 JAR 18개(`1.1.6` 기준)만 담고, 생성되는 `README.md`가 맨 앞에서 그 사실과 공식 배포처에서 직접 받아 넣는 절차를 안내하며 MultiMC 사용자에게 `.mrpack`을 권한다. `bundle_jar=false`인데 CDN URL이 없으면 설치 경로가 없으므로 빌드를 중단한다.
...
`1.1.6` 추가 사항: 서버 lock에 고정한 Simple Tomb `1.9.0`(CurseForge 파일 `8925463`)을 빌더의 `CLIENT_TITLES`와 `LICENSES`(LGPL-2.1)에 더해 서버 공통 모드가 14개가 됐다. Modrinth CDN URL이 없으므로 `.mrpack`에는 `client-overrides/mods/`로 JAR을 담는다. 서버는 GitHub 사전 릴리스 `client-1.1.6` 공개와 사이트 배포 뒤 2026-09-25에 재시작해 모드를 활성화했다(`1.1.5` 이하 클라이언트는 접속 불가). 현재 검증은 `tests/test_server_mods.py`, `tests/test_client_pack.py`다. `1.1.6` 아카이브로 새로 만든 인스턴스의 게임 실행은 확인하지 않았다.
...
`1.1.7` 추가 사항: 모드·셰이더·서버 목록은 `1.1.6`과 같다. 입력 lock의 `resourcepacks`에 Occultism 한국어 번역, Stay True, Vanilla Experience+를 기록한다. 클라이언트 빌더는 `scripts/build_occultism_resource_pack.py`를 먼저 실행해 번역 ZIP을 새로 만들며, 번역과 Stay True는 세 아카이브에 담고 Vanilla Experience+는 `.mrpack`의 공식 CDN 다운로드로 설치한다. 외부 팩의 크기·해시·ZIP·팩 메타데이터를 검증하며, 수동 ZIP과 MultiMC의 `manifest.json`에도 포함된 리소스팩을 기록한다. 새 인스턴스의 `options.txt`는 `lang:ko_kr`, `resourcePacks:["vanilla","mod_resources","file/occultism-ko-1.256.0-mc26.3.zip"]`를 사용한다. NeoForge가 모드 리소스를 번역보다 높은 위치에 자동 삽입하지 않도록 순서를 명시한다. 선택 팩 두 개는 꺼진 상태다. 검증은 `tests/test_client_resourcepacks.py`와 기존 클라이언트·번역 검사다. Stay True는 공식 최신판이 1.21.5용이며 26.3 호환 및 새 클라이언트의 게임 실행은 확인하지 않았다.

`1.1.8` 추가 사항: 서버 lock의 Modonomicon을 공식 Modrinth 버전 `SFKjMnCe`(`26.3-2.7.0`)로 바꿨고 나머지 모드·셰이더·리소스팩·`options.txt`·`servers.dat`는 `1.1.7`과 같다. 2.7.0은 26.3의 SDL 마우스 버튼 번호 때문에 책 노드 화면을 왼쪽 버튼으로 끌지 못하던 문제의 상류 수정 `d74b6f2dbba9956182f11808f82e1a22911cb5ee`를 담는다(게임 내 확인 안 함). 서버 교체 전(2026-09-25) 산출물은 빌더 모듈을 불러와 `SERVER_MODS`를 임시 디렉터리(나머지 22개 JAR 심볼릭 링크 + 해시를 확인한 2.7.0 JAR)로 바꾼 뒤 `main()`을 호출해 만들었다. 번역 빌더는 계속 운영 `mods/`의 Occultism JAR을 읽는다. `client-1.1.8`로 공개한 아카이브는 이 빌드다. 운영 `mods/`가 2.7.0으로 바뀐 지금은 인자 없는 빌더 실행으로 같은 내용의 산출물을 다시 만들 수 있지만(아카이브 항목의 ZIP 시각은 빌드 시각이라 바이트 단위로는 달라진다), 공개한 산출물을 덮어쓰므로 새 판을 만들 때만 실행한다. 서버 교체 뒤 검사가 통과했으며 현재 서버·클라이언트 검증은 `tests/test_server_mods.py`, `tests/test_client_pack.py`, `tests/test_multimc_pack.py`로 통합했다. 서버 교체 기록은 [모드 설치 안내](MODS_26_3.md#modonomicon-270-교체-2026-09-25)에 있다.

`1.1.9` 추가 사항: 서버 lock에 Traveler's Backpack `11.4.0`(Modrinth `Gdy0zkAN`)을 더해 24개가 됐고, 빌더의 `CLIENT_TITLES`와 `LICENSES`(LGPL-3.0-only, 제작자 모드팩 사용 허가)에 더해 서버 공통 모드가 15개, 클라이언트 모드가 20개가 됐다. Modrinth CDN URL이 있어 `.mrpack`은 다운로드 항목으로 받고, 수동·MultiMC ZIP은 JAR을 담는다(19개). 서버 설치 전(2026-09-25)에는 운영 `mods/`에 없었으므로 1.1.8 때와 같이 빌더 모듈을 불러와 `SERVER_MODS`를 임시 디렉터리(운영 JAR 23개 심볼릭 링크 + 해시를 확인한 Traveler's Backpack JAR)로 바꾼 뒤 `main()`을 호출해 빌드했다. `client-1.1.9`로 공개한 아카이브가 이 빌드다. 서버 설치 뒤 전체 검사가 통과했으며 현재 무결성 검사는 `tests/test_server_mods.py`, `tests/test_client_pack.py`, `tests/test_multimc_pack.py`로 통합했다. 설치 기록은 [모드 설치 안내](MODS_26_3.md#travelers-backpack-추가-2026-09-25)에 있다. 배낭 열기의 상류 기본값 `B`가 Occultism·Tom's Simple Storage·JourneyMap과 겹쳐, 빌더의 `BACKPACK_KEY_MAPPING`·`BACKPACK_KEY`로 새 인스턴스 `options.txt`에 이 키 하나만 `key.keyboard.y`로 적는다(클라이언트 lock `seeded_options.rebound_key_mappings`). 근거는 [클라이언트 모드 호환성 검토](CLIENT_MOD_COMPATIBILITY_26_3.md#119-travelers-backpack-단축키-중복)에 적었다.

`1.1.10` 추가 사항: 입력 lock에 클라이언트 전용 Nemo's Inventory Sorting `26.3-1.22.1`(Modrinth `JHgf35QL`/`aeA0nhgf`)을 더해 클라이언트 전용 모드가 6개, 전체 21개가 됐다. JAR 안 라이선스(All rights reserved)가 재배포를 금지하므로 `bundle_jar=false`이며, JourneyMap처럼 `.mrpack`은 공식 CDN 다운로드, 수동·MultiMC ZIP은 JAR 제외와 README 수동 설치 안내다(ZIP에 담는 JAR은 계속 19개). 빌더의 `NEMO_SETTINGS`·`render_nemo_config`가 `config/nemos-inventory-sorting/general.json`을 세 아카이브(`client-overrides/`, 루트, `.minecraft/`)에 넣어 Nemo의 끌기·분할·휠 이동을 끄며(기존 Mouse Tweaks 조작은 그대로), 클라이언트 lock `seeded_configs`에 기록한다. `options.txt`·`servers.dat`와 기존 JAR·리소스팩 바이트는 1.1.9와 같다. 서버 lock·서버 `mods/`는 바꾸지 않는다. 검증은 `tests/test_nemo_inventory_sorting.py`와 기존 클라이언트 검사다. `client-1.1.10`으로 공개했고(자산 6개 크기·SHA-256 확인, 사이트 배포 확인) 게임 실행·버튼 동작·전용 화면 호환은 확인하지 않았다. 근거는 [클라이언트 모드 호환성 검토](CLIENT_MOD_COMPATIBILITY_26_3.md#1110-nemos-inventory-sorting과-mouse-tweaks)에 있다.
