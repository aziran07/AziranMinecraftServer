...
`https://aziran.uk`의 접속 안내는 `site/`의 정적 페이지를 GitHub Pages로 배포한다. 게임 접속은 `mc.aziran.uk:25565`를 통해 Minecraft 컨테이너로 직접 연결한다. 사이트 배포 워크플로는 `.github/workflows/pages.yml`, 안내 테스트는 `tests/test_join_guide.py`다. 현재 공개된 클라이언트는 GitHub 사전 릴리스 [`client-1.1.7`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.7)이다. Modonomicon 2.7.0을 담은 `1.1.8`은 로컬에서 빌드했고 사이트 소스도 `client-1.1.8` 링크로 바꿨지만 Release는 아직 만들지 않았다. 1.1.7 사이트 소스도 작업 브랜치의 Pages 배포가 환경 보호 규칙에 거부되어 PR #7의 `main` 병합 후 배포 확인이 필요하다. 실험 단계 Iris 때문에 사전 릴리스로 제공하며 이전 릴리스는 유지한다. 상세 운영 절차는 [웹사이트 운영 안내](JOIN_GUIDE.md)를 참고한다.
...
`1.1.8` 추가 사항: 서버 lock의 Modonomicon을 공식 Modrinth 버전 `SFKjMnCe`(`26.3-2.7.0`)로 바꿨고 나머지 모드·셰이더·리소스팩·`options.txt`·`servers.dat`는 `1.1.7`과 같다. 2.7.0은 26.3의 SDL 마우스 버튼 번호 때문에 책 노드 화면을 왼쪽 버튼으로 끌지 못하던 문제의 상류 수정 `d74b6f2dbba9956182f11808f82e1a22911cb5ee`를 담는다(게임 내 확인 안 함). **서버 lock은 2.7.0을 가리키지만 운영 `server-data-26.3-neoforge/mods/`에는 서버 교체 전까지 2.6.0이 남아 있다.** 이 상태에서 빌더를 그대로 실행하면 2.7.0 JAR이 없어 중단된다. 서버 교체 전 산출물은 빌더 모듈을 불러와 `SERVER_MODS`를 임시 디렉터리(나머지 22개 JAR 심볼릭 링크 + 해시를 확인한 2.7.0 JAR)로 바꾼 뒤 `main()`을 호출해 만들었다. 번역 빌더는 계속 운영 `mods/`의 Occultism JAR을 읽는다. 서버 교체 뒤에는 인자 없는 빌더 실행으로 같은 내용의 산출물을 다시 만들 수 있다(아카이브 항목의 ZIP 시각은 빌드 시각이라 바이트 단위로는 달라진다). 서버 쪽 검증은 `tests/test_modonomicon_update.py`이며 서버 교체 전에는 서버 검사가 실패하는 것이 정상이다.
