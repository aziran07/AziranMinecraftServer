...
- Modonomicon의 화면·설정·명령·연구 안내와 예제 본문을 원문과 대조했다. 기존 Occultism의
  세 번역 파일은 변경하지 않았다.
- 확장 작업 당시 로컬 클라이언트 아카이브와 생성 lock도 새 번역으로 다시 만들었다.
  당시 버전명은 `1.1.10`이었으나 기존 공개 릴리스는 교체하지 않았다.
  사용자 적용 확인과 릴리스 요청에 따라 새 배포 버전은 `1.1.11`로 구분한다.
- 기존 팩 활성화 ID는 유지했다. 사용자가 기존 게임 인스턴스에서 새 번역이 정상 적용됨을 확인했다(2026-09-25).
  모든 페이지의 줄바꿈·글자 잘림·링크 이동이나 새 1.1.11 인스턴스 전체 설치를 검증했다는 뜻은 아니다.

### 1.1.11 공개

2026-09-25 15:08 UTC [client-1.1.11](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.11)을 일반 릴리스(latest)로 공개했다. 세 클라이언트 아카이브·번역 ZIP·설치 문서·라이선스·체크섬의 7개 자산을 다시 내려받아 크기와 SHA-256을 확인했다. 번역 ZIP은 위에서 사용자가 적용을 확인한 바이트와 같다. 웹사이트도 1.1.11 링크로 배포하고 확인했으며, 상세 기록은 [웹사이트 운영 안내](JOIN_GUIDE.md)에 있다.
