...
형식, 클라이언트 팩은 바꾸지 않으며 추가 네트워크 메시지나 레지스트리를 만들지
않는다. 실제 클라이언트 접속과 화면 조작 확인은 자동 검사와 별도로 기록한다.
...
- 배포 검사: `tests/test_backpack_persistence_config.py`는 연동 활성화,
  ...
  클라이언트 팩 제외를 검사한다. 실제 플레이어 접속 검사를 대신하지 않는다.
...
- 기존 클라이언트 팩·릴리스 파일은 바꾸지 않았다. 이후 빌드에서도 패치를
  제외하도록 클라이언트 빌더의 서버 전용 제외 목록에 등록했다.
