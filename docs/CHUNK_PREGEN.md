...
- 캠페인 시작 당시에는 `server-data-26.3-neoforge/`의 정기 백업 정책이 없었고 기존 `mc_backup.sh`는 `server-data/world`를 대상으로 했다. 캠페인 시작 전 별도 일회성 백업을 만들었다(아래 기록). 이후 26.3 월드 정기 백업은 [월드 백업 안내](BACKUPS.md)를 따른다.
...
`scripts/chunky_idle_controller.py`는 접속자가 0명일 때만 목표 반경까지 프리젠을 진행한다. Compose 서비스 `chunky-idle-pregen`(컨테이너 `aziran-chunky-idle-pregen`)이 공식 Python 이미지 `python:3.13-alpine`의 `python3`로 실행한다(사용자 `1000:1000`). 표준 라이브러리만 쓰며 RCON 비밀번호는 `RCON_PASSWORD` 환경변수에서만 읽고 로그에 남기지 않는다.
...
1. RCON `list`로 접속자를 확인한다. 캠페인 작업이 진행 중이고 이 프로세스가 pause를 확인하지 않은 동안(시작·재개 직후, 컨트롤러 재시작 직후, RCON 재연결 대기 중 포함)은 약 1초(`--poll-interval`, 기본 1)마다, 플레이어 때문에 pause가 확인됐거나 아직 시작한 작업이 없어 시작을 기다리는 동안은 약 60초(`--idle-poll-interval`, 기본 60)마다 확인한다. 두 값은 0 이하·NaN·무한대를 거부해 바쁜 반복을 막는다. RCON 연결은 계속 유지해 폴링마다 새 접속 로그가 생기지 않게 한다.
...
4. 플레이어를 본 뒤에는 `--resume-delay`(기본 30초) 동안 접속자가 계속 0명이어야 재개한다. 재접속이 잦을 때 시작·정지가 반복되지 않게 하기 위한 값이다. 지연은 마지막으로 플레이어를 본 폴링 시각부터 세며, pause된 동안에는 60초 간격으로만 확인한다. 기본값(60초 간격 > 30초 지연)에서는 퇴장 뒤 처음 0명을 본 60초 폴링에서 지연이 이미 지나 있으므로, 재개는 마지막 플레이어가 나간 뒤 최대 약 60초(+ RCON 왕복 시간) 안에 일어나고 퇴장 직후 폴링이 돌면 30초보다 빨리 재개될 수도 있다. 간격을 지연보다 짧게 설정하면 기존처럼 마지막 확인 후 `--resume-delay`가 지난 뒤 첫 폴링에서 재개한다.
5. 컨트롤러가 SIGTERM(`docker compose stop`)이나 오류로 끝날 때는 현재 대상을 pause한 뒤 종료한다.

### 일시정지 지연(유한함)

즉시 정지가 아니라 폴링 기반이다. 작업이 진행 중일 때는 1초 간격으로 확인하므로, 플레이어가 `list`에 잡힌 뒤(로그인 완료 후)부터 최대 폴링 간격 1초 + RCON 왕복 시간 안에 `pause`를 보낸다. 60초 간격은 작업이 pause됐거나 시작 전이라 생성이 돌지 않는 동안에만 쓰므로 정지 지연에는 영향이 없다. Chunky는 pause를 받은 뒤 이미 요청한 청크 생성이 끝나야 작업 스레드를 멈추므로, 접속 직후 몇 초 동안은 생성 부하가 남을 수 있다. `list`가 0명을 반환한 직후 접속한 경우는 `execute unless entity @a` 조건이 서버에서 명령 실행과 같은 시점에 확인하므로 `start`/`continue`가 실행되지 않는다. 다만 플레이어 엔티티가 생기기 전(로그인 처리 중)에는 조건이 0명으로 보므로, 그 순간 시작된 작업은 다음 폴링에서 멈춘다.
...
- 캠페인 **시작 전 전체 데이터 백업**에는 `mc_backup.sh`와 `minecraft_backups/`를 쓰지 않는다. 현재 스크립트도 `world/`만 대상으로 하고 정기 백업을 자동 삭제한다. 전체 데이터는 별도 보호 경로에 둔다.
...
- 컨트롤러 이미지 `python:3.13-alpine`에는 HEALTHCHECK가 없어 `docker ps`에 health 표시가 없다. 컨트롤러 상태는 exit 코드(`restart: on-failure`), `RestartCount`, `docker compose logs chunky-idle-pregen`으로 확인한다. 이미지를 바꾼 뒤에는 `docker compose up -d --no-deps chunky-idle-pregen`으로 컨트롤러만 재생성한다(종료 시 pause, 새 컨트롤러가 접속자 0명을 확인한 뒤 continue).
