...
**`1.1.9`는 준비 단계입니다.** 세 아카이브를 빌드했고 [서버 고정 목록](mods-26.3.lock.json)에 Traveler's Backpack을 더했지만, 운영 서버의 `mods/`에는 아직 설치하지 않았습니다. 순서는 GitHub 초안(draft) 릴리스 `client-1.1.9`에 자산을 올려 해시 확인 → 월드만 백업한 뒤 서버에 설치·재시작 → 서버 기동 확인 뒤 초안을 **일반 릴리스(latest)**로 공개입니다. `1.1.9`부터는 사전 릴리스가 아닌 일반 릴리스로 공개하지만, 셰이더용 Iris는 여전히 미병합 PR #3354(커밋 `10d3598`)의 로컬 빌드이며 공식 안정판이 아닙니다. 이전 판은 사전 릴리스로 그대로 둡니다. 서버에 이 모드를 넣으면 이 모드가 없는 `1.1.8` 이하 인스턴스로는 접속할 수 없으므로 `1.1.9`를 새 인스턴스로 가져오도록 안내합니다. 사이트 소스는 `client-1.1.9` 링크로 바꿨지만 Pages 환경 보호 규칙 때문에 배포되지 않은 상태입니다. `1.1.9` 인스턴스의 게임 실행과 배낭 사용은 게임에서 확인하지 않았습니다.
