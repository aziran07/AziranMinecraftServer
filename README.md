...
`1.1.11`은 릴리스 [`client-1.1.11`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.11)로 공개할 산출물을 로컬에서 준비했으며, **아직 공개하지 않았습니다.** 릴리스에는 세 아카이브, 설치·라이선스 문서, 번역 팩 단독 ZIP `occultism-ko-1.256.0-mc26.3.zip`(SHA-256 `206fe5460f3cbe3bc84ace4a981ec5924dd52301dddf605895c992409e091c0d`)과 이 6개 파일의 `SHA256SUMS.txt`를 올립니다. 사용자가 새 번역 팩을 기존 게임 인스턴스에 넣어 번역이 적용되는 것을 확인했습니다. `1.1.11` 아카이브로 새로 만든 인스턴스의 실행과 모든 책 페이지·링크는 확인하지 않았습니다. 서버는 바뀌지 않았으므로 서버 재시작이 필요 없고, `1.1.10` 인스턴스도 그대로 접속할 수 있습니다.

`1.1.10`은 GitHub **일반 릴리스**(1.1.11 이전 latest) [`client-1.1.10`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.10)로 2026-09-25 14:16 UTC 공개했습니다. 소스 커밋 `f65d6c2`에 태그만 먼저 올리고, 초안 릴리스에 올린 6개 자산을 내려받아 크기·SHA-256이 로컬 산출물과 일치함을 확인한 뒤 공개했습니다. 그다음 `main`을 올려 Pages 배포 실행 [`36146375828`](https://github.com/aziran07/AziranMinecraftServer/actions/runs/36146375828)이 성공했고, `https://aziran.uk`가 HTTP 200으로 `client-1.1.10` 다운로드 링크·팩 이름·모드 21개·정렬 안내 절을 제공하며 링크된 `.mrpack`(HTTP 200, SHA-256 일치)과 배포 페이지가 열리는 것을 확인했습니다. 추가한 모드는 클라이언트 전용이라 서버 구성은 바뀌지 않으므로 서버 재시작·백업이 필요 없고, `1.1.9` 인스턴스도 그대로 접속할 수 있습니다(서버 쪽 조건이 같다는 판단이며 접속 시험은 하지 않았습니다). 기존 인스턴스는 자동으로 업데이트되지 않습니다. `1.1.10` 인스턴스의 게임 실행, 정렬 버튼 동작, 배낭·저장고 같은 모드 전용 화면에서의 호환은 게임에서 확인하지 않았습니다.

`1.1.9`는 GitHub **일반 릴리스**(1.1.10 이전 latest) [`client-1.1.9`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.9)로 2026-09-25 공개했습니다. 초안에 올린 6개 자산의 크기·SHA-256을 로컬 산출물과 대조한 뒤, 서버에 Traveler's Backpack을 설치하고 기동을 확인하고 나서 공개했습니다. 서버는 같은 날 월드만 보호 백업한 뒤 설치했습니다(13:44:54 UTC 정상 종료, 13:45:43 UTC 시작, 13:45:59 UTC healthy). 기록은 [모드 설치 안내](docs/MODS_26_3.md#travelers-backpack-추가-2026-09-25)에 있습니다. `1.1.9`부터는 사전 릴리스가 아닌 일반 릴리스로 공개하지만, 셰이더용 Iris는 여전히 미병합 PR #3354(커밋 `10d3598`)의 로컬 빌드이며 공식 안정판이 아닙니다. 이전 판은 사전 릴리스로 그대로 둡니다. 서버에 이 모드가 들어갔으므로 이 모드가 없는 `1.1.8` 이하 인스턴스로는 접속할 수 없고, `1.1.9`를 새 인스턴스로 가져오도록 안내합니다(`1.1.8` 이하 인스턴스의 실제 접속 거부는 시험하지 않았습니다). 사이트 소스는 `client-1.1.9` 링크로 바꿨지만 작업 브랜치에서는 Pages 환경 보호 규칙 때문에 배포되지 않았고, PR #7을 `main`에 병합(`0a0056b`)한 뒤 [배포 실행 `36144033527`](https://github.com/aziran07/AziranMinecraftServer/actions/runs/36144033527)이 성공해 해결됐습니다(2026-09-25). `1.1.9` 인스턴스의 게임 실행, 배낭 화면·Curios 착용·`Y` 단축키, 사망 시 배낭 처리는 게임에서 확인하지 않았습니다.

`1.1.8`은 GitHub **사전 릴리스** [`client-1.1.8`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.8)으로 2026-09-25 공개했습니다. 세 아카이브와 설치·라이선스 문서·체크섬 6개 자산의 크기·SHA-256이 로컬 산출물과 일치함을 확인했습니다. 서버도 같은 날 Modonomicon을 `2.7.0`으로 교체했습니다(13:13:42 UTC 정상 종료, 보호 백업 뒤 13:18:47 UTC 시작, 13:19:06 UTC healthy). 교체 기록은 [모드 설치 안내](docs/MODS_26_3.md#modonomicon-270-교체-2026-09-25)에 있습니다. 사이트 소스도 `client-1.1.8` 링크로 바꿨지만 아래 `1.1.7`과 같은 Pages 환경 보호 규칙 때문에 당시에는 사이트가 배포되지 않았습니다(PR #7 병합 뒤 해결). 2.7.0 서버에 `1.1.7`(2.6.0) 이하 클라이언트가 접속되는지는 확인하지 않았으므로 당시 `1.1.8`을 새 인스턴스로 가져오도록 안내했습니다. `1.1.8` 인스턴스의 게임 실행과 책 화면 끌기 수정도 게임에서 확인하지 않았습니다.

`1.1.7`은 GitHub **사전 릴리스** [`client-1.1.7`](https://github.com/aziran07/AziranMinecraftServer/releases/tag/client-1.1.7)으로 공개했습니다. 세 클라이언트 아카이브와 설치·라이선스 문서·체크섬을 올렸고, GitHub의 파일 크기·SHA-256이 로컬 산출물과 일치함을 확인했습니다. 가이드 소스는 새 다운로드 링크와 리소스팩 안내를 포함하지만 당시에는 사이트 배포를 완료하지 못했습니다. [배포 실행](https://github.com/aziran07/AziranMinecraftServer/actions/runs/36117840610)이 `github-pages` 환경의 브랜치 보호 규칙에 거부됐습니다. 허용 브랜치는 `main`과 `feat/join-guide`이며 당시 작업 브랜치 `feat/occultism-ko-translation`은 제외되어 있었습니다. PR #7을 `main`에 병합(`0a0056b`)한 뒤 [배포 실행 `36144033527`](https://github.com/aziran07/AziranMinecraftServer/actions/runs/36144033527)이 성공해 해결됐습니다. 보호 규칙은 변경하지 않았습니다.
...
### 먼저 읽을 것: JourneyMap, Nemo's Inventory Sorting, 셰이더 팩, Vanilla Experience+는 ZIP 두 개에 들어 있지 않습니다

아래 네 파일은 **수동 ZIP과 MultiMC 인스턴스 ZIP에 담겨 있지 않습니다.** 라이선스가 모드팩에 파일을 직접 담는 것을 금지하고, CurseForge·Modrinth 같은 공식 배포처에서 내려받는 방식만 허용하기 때문입니다.

- JourneyMap `26.3-6.0.9+neoforge` — [공식 라이선스](https://teamjm.github.io/journeymap-docs/6.0.x/about/licensing/)가 번들·재호스팅을 금지합니다.
- Nemo's Inventory Sorting `26.3-1.22.1` — JAR 안의 [라이선스](https://github.com/NemoNotFound/NemosInventorySorting?tab=License-1-ov-file)(All rights reserved)가 원본·수정본 재배포와 재업로드를 금지하고 공식 배포처(CurseForge, Modrinth, GitHub)에서만 받도록 요청합니다.
...
Occultism 한국어 번역 팩 `occultism-ko-1.256.0-mc26.3.zip`만 새로 만들었습니다. 모드 21개(같은 JAR), 셰이더 팩, 리소스팩 3개와 기본 활성화, `options.txt`, `servers.dat`, Nemo 설정 파일은 `1.1.10`과 같고, 번역 팩의 파일 이름과 활성화 ID(`file/occultism-ko-1.256.0-mc26.3.zip`)도 그대로입니다. 자세한 번역 범위와 검증은 [번역 문서](docs/OCCULTISM_KO_TRANSLATION.md)에 있습니다.

- Occultism 안내서 페이지에 직접 적힌 영어 본문 5개(내세나무 묘목, 통나무, 자연산 변종, Otherstone, Otherrock)를 번역하고, 안내서가 참조하지만 원문 언어 파일에 없는 키 4개를 보완했습니다. Occultism 키는 4,341개입니다.
- 같은 팩에 Modonomicon `2.7.0` 영어 키 309개 전체의 한국어 번역(`assets/modonomicon/lang/ko_kr.json`)을 더했습니다.
- **확인한 것과 하지 않은 것:** 사용자가 새 번역 팩을 기존 게임 인스턴스에 넣어 번역이 적용되는 것을 확인했습니다. `1.1.11` 아카이브로 새로 만든 인스턴스의 실행과 모든 책 페이지·링크는 확인하지 않았습니다.

### 1.1.10: Nemo's Inventory Sorting 추가

클라이언트 전용 모드 [Nemo's Inventory Sorting](https://modrinth.com/mod/nemos-inventory-sorting) `26.3-1.22.1`(Modrinth 버전 `aeA0nhgf`, `nemos-inventory-sorting-NeoForge-26.3-1.22.1.jar`, 169421바이트)을 더했습니다. 인벤토리·상자 화면에 **정렬(`Sort`)**, **같은 아이템 옮기기(`Move Same`)**, 모두 옮기기, 모두 버리기 버튼과 `Alt` 슬롯 잠금, `Ctrl`+`F` 아이템 검색을 더합니다. JAR의 `neoforge.mods.toml`이 선언하는 필수 의존성은 Minecraft `[26.3,)`과 NeoForge `[26.3.0.1-beta,)`뿐이고 번들 JAR이 없습니다(모드 ID `nemos_inventory_sorting`). Modrinth는 `client_side=required`, `server_side=unsupported`로 표시하며 서버에는 설치하지 않습니다. 라이선스 때문에 JAR은 어느 아카이브에도 담지 않았고, `.mrpack`은 Modrinth CDN에서 원본을 받습니다. ZIP 두 개는 팩 안 `README.md`가 받는 절차와 SHA-512를 안내합니다.

- **핫바는 기본으로 빠집니다.** 내 인벤토리 쪽 버튼을 그냥 누르면 핫바 9칸은 건드리지 않고, Shift를 누른 채 누르면 핫바까지 포함합니다.
- **Mouse Tweaks는 그대로 둡니다.** Nemo도 Shift+끌기 빠른 이동, Shift+우클릭 절반 이동, 휠 이동을 기본으로 켜므로, 세 아카이브에 `config/nemos-inventory-sorting/general.json`을 담아 이 셋(`enableDragQuickMove`·`enableSplitQuickMove`·`enableScrollTransfer`)을 `false`로 껐습니다. 기존 Mouse Tweaks 조작은 바뀌지 않습니다. 같은 파일에서 `includeHotbarByDefault=false`, `enableSlotLocking=true`, 단축키 등록 세 항목을 `true`로 둡니다. 필드 이름과 경로는 JAR의 클래스와 상류 소스 커밋 [`9deb444`](https://github.com/NemoNotFound/NemosInventorySorting/tree/9deb444440caf3b9a69579166b72f39cc3dc8d20)의 `SettingsConfig`·`ConfigService`로 확인했습니다.
- 정렬·옮기기·버리기 단축키는 기본으로 지정되어 있지 않고(빠른 검색만 `F`), `options.txt`는 `1.1.9`와 같습니다. 필요하면 `Controls` → `Key Binds`의 `Nemo's Inventory Sorting` 항목에서 지정합니다.
- 모드는 설정 파일이 **없을 때만** 기본값으로 만듭니다. 이미 만든 인스턴스에 더할 때는 JAR을 `mods/`에 넣고 팩의 `config/nemos-inventory-sorting/general.json`도 같은 경로에 넣어야 하며, 이미 생긴 파일이 있으면 덮어쓰거나 세 이동 항목과 `includeHotbarByDefault`를 `false`로 고칩니다. 권장은 `1.1.10`을 새 인스턴스로 가져오는 것입니다.
- **확인하지 않은 것:** 버튼 위치와 동작, Traveler's Backpack 배낭 화면·Tom's Simple Storage 터미널·Occultism 저장고 같은 모드 전용 화면에서의 호환. 제작자가 호환을 밝힌 모드(Iron Chests 등)는 이 팩에 없습니다.
...
| 리소스팩 | 출처 | 팩 포함 | 기본 상태 |
| --- | --- | --- | --- |
| Occultism 한국어 번역 `occultism-ko-1.256.0-mc26.3.zip` | 이 저장소([번역 문서](docs/OCCULTISM_KO_TRANSLATION.md)), MIT(원작 Occultism), `1.1.11`부터 Modonomicon 번역은 CC-BY-SA-4.0 | 세 아카이브에 포함 | **켜짐** |
...
MultiMC 인스턴스 ZIP의 인스턴스 이름은 `Aziran 26.3 Client 1.1.11`입니다. Minecraft `26.3`과 NeoForge `26.3.0.8-beta`는 `mmc-pack.json`의 컴포넌트로만 지정해 런처가 공식 메타데이터에서 내려받으므로 첫 실행에 인터넷 연결과 로그인한 계정이 필요합니다. Java `25` 경로는 `Edit Instance` → `Settings` → `Java`에서 직접 지정합니다.
