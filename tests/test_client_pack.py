...
class ClientPackTests(unittest.TestCase):
    ...
    def test_artifacts_match_requested_mods_and_dependencies(self):
        ...
        self.assertEqual(lock['pack_version'], '1.1.7')
        self.assertEqual(lock['mod_count'], 19)
        self.assertEqual(lock['shared_with_server_count'], 14)
        ...
        manual = ROOT / 'dist/aziran-26.3-client-1.1.7-manual.zip'
        ...
        with zipfile.ZipFile(ROOT / 'dist/aziran-26.3-client-1.1.7.mrpack') as archive:
            self.assertIsNone(archive.testzip())
