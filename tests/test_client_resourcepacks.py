"""Client 1.1.8 must install the requested packs and enable only the translation."""
...
class ClientResourcePackTests(unittest.TestCase):
    ...
    def test_pack_delivery_and_defaults(self):
        ...
        self.assertEqual([p['filename'] for p in packs if p['enabled_by_default']],
                         [TRANSLATION])
        archives = {
            'aziran-26.3-client-1.1.8.mrpack': 'client-overrides/',
            'aziran-26.3-client-1.1.8-manual.zip': '',
            'aziran-26.3-client-1.1.8-multimc.zip': '.minecraft/',
        }
