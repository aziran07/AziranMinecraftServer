"""Client 1.1.7 must install the requested packs and enable only the translation."""
...
class ClientResourcePackTests(unittest.TestCase):
    ...
    def test_pack_delivery_and_defaults(self):
        ...
        self.assertEqual([p['filename'] for p in packs if p['enabled_by_default']],
                         [TRANSLATION])
        archives = {
            'aziran-26.3-client-1.1.7.mrpack': 'client-overrides/',
            'aziran-26.3-client-1.1.7-manual.zip': '',
            'aziran-26.3-client-1.1.7-multimc.zip': '.minecraft/',
        }
